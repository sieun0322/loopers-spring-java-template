...
# --- 설정값 변경 ---
START_ID = 10001
TOTAL_ROWS_TO_GENERATE = 90000  # 10001번부터 90000개 생성 (마지막 ID는 100000)
TOTAL = START_ID + TOTAL_ROWS_TO_GENERATE - 1 # 최종 ID 번호 (100000)
BATCH_SIZE = 1_000
OUTPUT_FILE = "product-list-data-ranged.sql"
...
with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
    ...
    for batch_start in range(START_ID, START_ID + TOTAL_ROWS_TO_GENERATE, BATCH_SIZE):
        ...
        for i in range(batch_start, batch_end + 1):
            ...
            values.append(
                f"({i}, {brand_id}, '{name}', {price}, {stock_quantity}, {like_count}, '{NOW}', '{NOW}', NULL)"
            )

        sql = (
            "INSERT INTO product_list_view "
            "(product_id, brand_id, name, price, stock_quantity, like_count, created_at, updated_at, deleted_at) VALUES\n"
        )
        sql += ",\n".join(values)
        sql += ";\n\n"
        f.write(sql)
